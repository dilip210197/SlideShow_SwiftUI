...
public class VideoMaker: NSObject {
    ...
    public var gifFront: [UIImage?] = []
    public var gifBack: [UIImage?] = []
    ...
    public convenience init(images: [UIImage?], transition: ImageTransition, gifFront: [UIImage?], gifBack: [UIImage?]) {
        self.init()
        
        self.images = images
        self.transition = transition
        self.isMovement = false
        self.gifFront = gifFront
        self.gifBack = gifBack
    }
    ...
    public func exportVideo(audio: AVURLAsset?, audioTimeRange: CMTimeRange?, video: URL?, completed: @escaping CompletedCombineBlock) -> VideoMaker {
    ...
    fileprivate func performTransitionDrawing(cxt: CGContext?, from: CGImage, to: CGImage?, with transition: ImageTransition, rate: CGFloat) {
        ...
        var currentGifFrameIndexBack = 0
        var currentGifFrameIndexFront = 0
        
        // Draw gifBack frames
        if !gifBack.isEmpty {
            let gifFrame = gifBack[currentGifFrameIndexBack]
            currentGifFrameIndexBack = (currentGifFrameIndexBack + 1) % gifBack.count
            let gifRect = CGRect(x: 0, y: 0, width: size.width, height: size.height)
            cxt?.draw((gifFrame?.cgImage!)!, in: gifRect)
        }

        // Draw gifFront frames if 'to' image is nil
        if to == nil {
            if !gifFront.isEmpty {
                let gifFrame = gifFront[currentGifFrameIndexFront]
                currentGifFrameIndexFront = (currentGifFrameIndexFront + 1) % gifFront.count
                let gifRect = CGRect(x: 0, y: 0, width: size.width, height: size.height)
                cxt?.draw((gifFrame?.cgImage!)!, in: gifRect)
            }
            return
    ...
    private func drawGifFront(cxt: CGContext?) {
        var currentGifFrameIndexFront = 0
        if !gifFront.isEmpty {
            let gifFrame = gifFront[currentGifFrameIndexFront]
            currentGifFrameIndexFront = (currentGifFrameIndexFront + 1) % gifFront.count
